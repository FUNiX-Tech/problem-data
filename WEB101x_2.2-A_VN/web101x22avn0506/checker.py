...
from dmoj.utils.css_parser import parse_css, parse_inline_css, get_element_css_value
...
def check(process_output, judge_output, judge_input, point_value, submission_source, **kwargs):
  ...
  if input == "Thẻ h1 có inline style là color: white":
    if h1.style is not None and parse_inline_css(h1).get('color') == 'white':
      return CheckerResult(True, point_value, "")
    return CheckerResult(False, 0, "")
  ...
  if input == "Khai báo class pink-text cần có !important để ghi đè tất cả các khai báo khác":
    ...
    if css.get(".pink-text") is not None and " !important" in css.get(".pink-text"):
      return CheckerResult(True, point_value, "")
    return CheckerResult(False, 0, "")
  
  # criteria 6
  if input == "Thẻ h1 có chữ màu hồng (pink)":
    if get_element_css_value(soup, h1, 'color') == 'pink':
      return CheckerResult(True, point_value, "")
    return CheckerResult(False, 0, "")
